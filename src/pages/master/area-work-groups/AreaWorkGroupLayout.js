...
import { AreaWorkGroupApi } from '../../../api/MasterApi'
...
export default function AreaWorkGroupLayout() {
    ...
    useEffect(() => {
        api.getAreaWorkGroup().then(data => {
            setItems(data);
        });
        setRefresh(false)
    },[refresh, state]);
    ...
    const onSearch = () => {
        ...
        setItems([...items.filter(el => el.arwgDesc.toLowerCase().includes(searchValue.value.toLowerCase() || ''))])
    }
    ...
    return (
        <>
            ...
            <Table onSortChange={onSortChange} sort={sort}>
                <Head>
                    <Cell key='arwgCode' sortable>Code</Cell>
                    <Cell key='arwgDesc' sortable>Desc</Cell>
                    <Cell key='cityName' sortable>City</Cell>
                ...
                <Body defaultItems={items}>
                    {
                        (items || []).map(item => (
                            <Row key={item.arwgCode}>
                                <Cell>{item.arwgCode}</Cell>
                                <Cell>{item.arwgDesc}</Cell>
                                <Cell>{item.cities.cityName}</Cell>
                                <Cell>
                                    <ClayDropDown trigger={<ClayIcon className="inline-item inline-item-after" symbol="ellipsis-v"/>}>
                                        <ClayDropDown.ItemList>
                                        <ClayDropDown.Item onClick={() => navigate(`edit/${item.arwgCode}`)}>
                                            Edit
                                        </ClayDropDown.Item>
                                        <ClayDropDown.Item
