...
import { TemplateWorkOrderApi } from '../../../api/MasterApi'
...
export default function WorkOrderLayout() {
    ...
    useEffect(() => {
        api.getTewo(activePage-1,totalDataSize).then(data => {
    ...
    return (
        <>
            ...
            <Table onSortChange={onSortChange} sort={sort}>
                <Head>
                    <Cell key='tewoID' sortable>No</Cell>
                    <Cell key='tewoName' sortable>Name</Cell>
                    <Cell key='tewoTestaId' sortable>Task</Cell>
                ...
                <Body defaultItems={items}>
                    {
                        (items || []).map(item => (
                            <Row key={item.tewoId}>
                                <Cell>{item.tewoId}</Cell>
                                <Cell>{item.tewoName}</Cell>
                                ...
                                <Cell>
                                    <ClayDropDown trigger={<ClayIcon className="inline-item inline-item-after" symbol="ellipsis-v"/>}>
                                        ...
                                        <ClayDropDown.Item onClick={() => navigate(`edit/${item.tewoId}`)}>
                                            Edit
                                        </ClayDropDown.Item>
                                        <ClayDropDown.Item
