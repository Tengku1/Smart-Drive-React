...
import { CitiesApi } from '../../../api/MasterApi'
...
export default function CitiesLayout() {
    ...
    useEffect(() => {
        api.getCities(activePage-1, totalDataSize).then(data => {
    ...
    const onSearch = () => {
        ...
        setItems([...items.filter(el => el.cityName.toLowerCase().includes(searchValue.value.toLowerCase() || ''))])
    }
    ...
    return (
        <>
            ...
            <Table onSortChange={onSortChange} sort={sort}>
                <Head>
                    <Cell key='cityId' sortable>No</Cell>
                    <Cell key='cityName' sortable>City Name</Cell>
